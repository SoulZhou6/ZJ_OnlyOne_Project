...
@interface ZKTableViewCellLayout ()
...
@implementation ZKTableViewCellLayout
...
- (instancetype)initWithData:(ZKVideoObj *)data {
    ...
    if (self) {
        ...
        min_y = 18;
        min_w = [@"呵呵额" textSizeWithFont:[UIFont systemFontOfSize:15] limitWidth:min_view_w-2*margin-min_x].width;
        min_h = 15;
...
- (CGFloat)videoHeight {
    ...
    if (self.isVerticalVideo) {
        videoHeight = [UIScreen mainScreen].bounds.size.width * 0.6 * self.data.video_height.floatValue/self.data.video_width.floatValue;
    } else {
        videoHeight = [UIScreen mainScreen].bounds.size.width * self.data.video_height.floatValue/self.data.video_width.floatValue;
    }
    return videoHeight;
}
