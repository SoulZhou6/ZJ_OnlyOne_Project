...
@property (nonatomic, weak) id<ZKTableViewCellDelegate> delegate;
@property (nonatomic, strong) NSIndexPath *indexPath;

@end

@implementation ZKVideoCell
...
- (void)setLayout:(ZKTableViewCellLayout *)layout {
    _layout = layout;
...
//    [self.headImageView setImageWithURLString:layout.data.author. placeholder:[UIImage imageNamed:@"defaultUserIcon"]];
    [self.coverImageView setImageWithURLString:layout.data.thumbnail_url placeholder:[UIImage imageNamed:@"loading_bgView"]];
    self.nickNameLabel.text = @"呵呵哒";
//    layout.data.author.user_name;
...
- (void)playBtnClick:(UIButton *)sender {
    if ([self.delegate respondsToSelector:@selector(zk_playTheVideoAtIndexPath:)]) {
        [self.delegate zk_playTheVideoAtIndexPath:self.indexPath];
    }
}
