...
@interface ZKWebBrowseViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate>
...
- (void)loadWebURLSring:(NSString *)string{
    ...
    if(string.length>1){
        ...
        ZKEmptyView *emptyView = [[ZKEmptyView alloc] initWithFrame:self.view.bounds titlt:@"链接失效"];
...
-(void)webView:(WKWebView *)webView didFailProvisionalNavigation:(WKNavigation *)navigation withError:(NSError *)error{
    ...
    ZKEmptyView *emptyView = [[ZKEmptyView alloc] initWithFrame:self.view.bounds titlt:@"加载失败，请检查网络"];
    [webView addSubview:emptyView];
}
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {
        UIImage* backItemImage = [[UIImage imageNamed:@"zk_icon_return"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        UIImage* backItemHlImage = [[UIImage imageNamed:@"zk_icon_return"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
...
- (UIProgressView *)progressView{
    if (!_progressView) {
        ...
        // 设置进度条的色彩
        [_progressView setTrackTintColor:[UIColor colorWithHexString:@"#f6f6f6"]];
