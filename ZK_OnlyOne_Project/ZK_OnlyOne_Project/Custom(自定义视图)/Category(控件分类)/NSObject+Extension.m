...
#define ZK_COLOR_ARGS_KEY @"ZK_COLOR_ARGS_KEY"
#define ZK_IMAGE_ARGS_KEY @"ZK_IMAGE_ARGS_KEY"
...
- (id)performSelector:(SEL)selector withObjects:(const NSArray<id> *)objects
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if ([obj isKindOfClass:[NSString class]] && [obj isEqualToString:ZK_THEME_COLOR])
        {
        ...
        if ([argumentTypeString isEqualToString:@"@"])
        {
            ...
            if ([obj isKindOfClass:[NSDictionary class]])
            {
                ...
                for (NSString *key in keys)
                {
                    ...
                    if ([value isKindOfClass:[NSString class]] && [value isEqualToString:ZK_THEME_COLOR])
                    {
...
{
    [self addToThemeColorPoolWithSelector:selector objectsKey:ZK_COLOR_ARGS_KEY objects:objects];
}

- (void)addToThemeImagePoolWithSelector:(SEL)selector objects:(NSArray *)objects
{
    [self addToThemeColorPoolWithSelector:selector objectsKey:ZK_IMAGE_ARGS_KEY objects:objects];
}
...
- (void)addToThemeColorPoolWithSelector:(SEL)selector objectsKey:(NSString *)argsKey objects:(NSArray *)objects
{
    ...
    if ([argsKey isEqualToString:ZK_IMAGE_ARGS_KEY])
    {
    ...
    else if ([argsKey isEqualToString:ZK_COLOR_ARGS_KEY])
    {
...
{
    [self setThemeColor:color image:NULL];
}
...
- (void)setThemeColor:(UIColor *)color image:(UIImage *)themeImage
{
    ...
    for (NSMapTable *mapTable in [_themeColorPool copy]) {
        ...
        {
            // 不能是参数key
            if (![key isEqualToString:ZK_COLOR_ARGS_KEY] && ![key isEqualToString:ZK_IMAGE_ARGS_KEY])
            {
                objectKey = key;
                break;
            }
        ...
        if ([objectKey containsString:@":"])
        {
            ...
            while (key = [enumerator nextObject])
            {
                if ([key isEqualToString:ZK_IMAGE_ARGS_KEY])
                {
                    isImage = YES;
                    break;
                }
            ...
            if (isImage)
            {
                ...
                if (themeImage != nil)
                {
                    NSArray *args = [mapTable objectForKey:ZK_IMAGE_ARGS_KEY];
            ...
            {
                NSArray *args = [mapTable objectForKey:ZK_COLOR_ARGS_KEY];
...
- (void)removeFromThemeColorPoolWithSelector:(SEL)selector
{
    ...
    for (NSMapTable *subMapTable in [[self themeColorPool] copy])
    {
        ...
        while (key = [enumerator nextObject])
        {
            if (![key isEqualToString:ZK_COLOR_ARGS_KEY]) {
                objectKey = key;
                break;
            }
        }
